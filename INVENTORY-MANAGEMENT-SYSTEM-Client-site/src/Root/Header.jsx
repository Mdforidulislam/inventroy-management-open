  ...
  const navList = (
    <ul className="mt-2 mb-4 flex flex-col gap-2 lg:mb-0 lg:mt-0 lg:flex-row lg:items-center lg:gap-6">
      ...
      >
       <CiShop className="text-2xl" />
        <Link to='/shopecrease'>Create Shop</Link>
      </Typography>
      <Typography
        as="li"
        variant="small"
     ...
     <MdOndemandVideo className="text-2xl" />
        <Link to='/demovideo'>Watch Demo</Link>
      </Typography>
      <Typography
        as="li"
        variant="small"
    ...
    return (
      ...
      <div className="container mx-auto flex items-center justify-between text-blue-gray-900">
        ...
        >
          <h1 className="font-bold text-xl">24MTP</h1>
        </Typography>
        <div className="hidden lg:block">{navList}</div>
        <div className="flex items-center gap-x-1 hidden md:flex">
        {
              userInfo ? <div className="flex gap-4">
                 <div className="block">
                 <h1 className="block">{userInfo?.email}</h1>
                 <h1 className="block">{userInfo?.displayName}</h1>
                 </div>
                <Avatar src={userInfo?.photoURL} alt="avatar" />
                <Button fullWidth variant="text" size="sm" className="bg-blue-gray-100">
              <span><Link onClick={handleLogount} >Log Out</Link></span>
            </Button>
              </div> :  <Button fullWidth variant="text" size="sm" className="bg-blue-gray-100">
              <span><Link to='/login'>Ling in</Link></span>
            </Button>
